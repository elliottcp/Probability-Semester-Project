...
for(i in 1:n) {
  if(y < pdf(points[i,1])) rbind(dark_sample, points[i,])
  if(y > pdf(points[i,1])) rbind(light_sample, points[i,])
}
...
ggplot(data = dark_sample, aes(x = x, y = y)) +
  geom_point(dark_sample, col = "black") +
  geom_point(light_sample, col = "gray") +
