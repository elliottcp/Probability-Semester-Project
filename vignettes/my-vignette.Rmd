...
Perhaps it will be easiest to understand visually.  First, 1000 uniformly distributed coordinate pairs were generated.  Then, only about 650 points fall underneath the pdf curve.  These are highlighted in black.  The other 350 were rejected (gray).  
...
sample <- THEFUNCTION1D(pdf, n = 20, lower_bound = -10, upper_bound = 10, C = 0.06)
hist(sample)
```
...
sample <- THEFUNCTION1D(pdf, n = 60, lower_bound = 0, upper_bound = 100, C = 50)
hist(sample)
```
...
sample <- data.frame(THEFUNCTION1D(pdf, n = 10000, lower_bound = 0, upper_bound = 50, C = 0.5))
...
sample <- THEFUNCTION1D(pdf, n = 50000, lower_bound = 0, upper_bound = 1, C = 1)
hist(sample)
```
